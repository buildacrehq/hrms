...
import { Plus, MapPin, Users, X, Building2, ExternalLink, Pencil, Check } from 'lucide-react';

type Site = {
  id: string; name: string; address: string | null; mapsUrl: string | null;
  status: string; _count: { employees: number };
};
...
export default function SitesPage() {
  ...
  const [editForm, setEditForm] = useState({ name: '', address: '', mapsUrl: '' });
  ...
  const createM = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sites'] }); setShowForm(false); setForm({ name: '', address: '', mapsUrl: '' }); },
  });

  const updateM = useMutation({
    mutationFn: (id: string) => api.patch(`/admin/sites/${id}`, editForm),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sites'] }); setEditId(null); },
  });
  ...
  function startEdit(s: Site) {
    setEditId(s.id);
    setEditForm({ name: s.name, address: s.address ?? '', mapsUrl: s.mapsUrl ?? '' });
  }
      ...
      <div className="px-8 py-6 space-y-5">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            ...
            <SiteFormFields form={form} onChange={setForm} />
                ...
                <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow group">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {!isEditing && (
                        <button onClick={() => startEdit(s)}
                          className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-300 hover:text-blue-500 hover:bg-blue-50 transition-colors opacity-0 group-hover:opacity-100">
                          <Pencil size={12} />
                        </button>
                      )}
                    </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                      <SiteFormFields form={editForm} onChange={setEditForm} compact />
                      <div className="flex gap-2 pt-1">
                        <button onClick={() => updateM.mutate(s.id)} disabled={updateM.isPending || !editForm.name}
                          className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-40 transition-colors">
                          <Check size={11} />{updateM.isPending ? 'Saving…' : 'Save'}
                        </button>
                    ...
                    <>
                      ...
                      {s.status === 'ACTIVE' && (
                        <button
                          onClick={() => { if (confirm(`Deactivate "${s.name}"?`)) deactivateM.mutate(s.id); }}
...
function SiteFormFields({
  ...
  form: { name: string; address: string; mapsUrl: string };
  onChange: (f: { name: string; address: string; mapsUrl: string }) => void;
  compact?: boolean;
}) {
  ...
  return (
    <div className={compact ? 'space-y-2' : 'grid grid-cols-1 sm:grid-cols-3 gap-4'}>
      <div>
        {!compact && <label className="block text-xs font-semibold text-slate-600 mb-1.5">Site Name <span className="text-red-400">*</span></label>}
        <input value={form.name} onChange={e => onChange({ ...form, name: e.target.value })}
          placeholder="e.g. BA-OFFICE" className={inp} />
      </div>
      <div>
        {!compact && <label className="block text-xs font-semibold text-slate-600 mb-1.5">Address <span className="font-normal text-slate-400">(optional)</span></label>}
        <input value={form.address} onChange={e => onChange({ ...form, address: e.target.value })}
          placeholder="Full address" className={inp} />
      </div>
      <div>
        {!compact && <label className="block text-xs font-semibold text-slate-600 mb-1.5">Google Maps URL <span className="font-normal text-slate-400">(optional)</span></label>}
        <input value={form.mapsUrl} onChange={e => onChange({ ...form, mapsUrl: e.target.value })}
